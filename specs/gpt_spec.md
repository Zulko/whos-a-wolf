...
Each villager makes **k claims** (k is configurable per villager; typically 1–3). Each claim is a boolean statement about which villagers are werewolves (and possibly counts/parity constraints).

Goal: generate puzzles where the set of claims yields a **unique consistent assignment** of who is a werewolf.
...
Villager `i` utters `k_i` claims:

* `claims_by_speaker[i] = [C_i0, C_i1, ..., C_i(k_i-1)]`

Each claim `C` is a boolean formula over some subset of the variables `W[0..N-1]` (often excluding `W[i]`, but that is a generation constraint, not required by the logic).

### Truthfulness rule (generalized for 1–3 claims)

Define the **aggregate truth** for speaker `i`:

* `ALL_TRUE[i] = AND(claims_by_speaker[i])`

Truthfulness constraint:

* Human: all claims true ⇒ `W[i] = False` ⇒ `ALL_TRUE[i] = True`
* Werewolf: at least one claim false ⇒ `W[i] = True` ⇒ `ALL_TRUE[i] = False`
...
> Optional design variant (not required): enforce that wolves also say at least one true claim. If adopted later, it becomes:
> `W[i] => (NOT ALL_TRUE[i]) AND (OR claims_by_speaker[i])`

---

## Puzzle coherence and uniqueness

A candidate puzzle is *coherent* if there exists at least one assignment `W` satisfying the truthfulness constraints for all villagers.
...
```text
class Claim:
    claim_id: str                 # stable canonical string (for hashing, cache keys)
...
### Relationship claims (two villagers)

All pairwise claims have indices `a_index`, `b_index` and normally represent formulas like implication, equivalence, xor, etc.
...
```text
class IfAThenB(RelationshipClaim):
    # semantics: W[a] => W[b]
...
class IfNotAThenB(RelationshipClaim):
...
* generation may forbid claims referencing the speaker (“no self-reference”)

### Count/parity claims (global or scoped)

Count claims refer to how many werewolves exist among a scope of indices.

```text
class CountClaim(Claim):
...
```text
class ExactlyKWerewolves(CountClaim):
    count: int
    # semantics: SUM(W[i] for i in scope) == count

class AtMostKWerewolves(CountClaim):
    count: int
    # semantics: SUM(...) <= count

class AtLeastKWerewolves(CountClaim):
    count: int
    # semantics: SUM(...) >= count

class EvenNumberOfWerewolves(CountClaim):
    # semantics: SUM(...) % 2 == 0

class OddNumberOfWerewolves(CountClaim):
    # semantics: SUM(...) % 2 == 1
```
...
We want a stable `claim_id` such that it uniquely identifies a claim instance. Examples:
...
We support a string-to-claim factory:
...
class Puzzle:
    ...
    claims_by_speaker: list[list[Claim]]       # length N; each inner list length 1..3
    # optional metadata
...
Since N=6 initially, there are `2^N = 64` possible assignments. We can precompute truth values for each claim and represent sets of assignments as bitmasks (integers).
...
### Precomputed claim truth masks

For each atomic claim `c` we compute:

* `truth_mask[c]`: bitmask with bit `idx` = 1 iff claim is TRUE under that assignment
...
class ClaimTruthTableCache:
    N: int
    claim_id_to_truth_mask: dict[str, int]

    def build_for_claim_library(claim_library: list[Claim]) -> ClaimTruthTableCache
    def save_to_json(path: str)
    def load_from_json(path: str) -> ClaimTruthTableCache
```
...
Generate `claims_by_speaker` such that:

* the puzzle is coherent
* it has exactly one solution (unique `W*`)
* each speaker has 1..3 claims (often exactly 2, configurable)
* optional: style constraints (no duplicates, avoid trivial claims, etc.)
...
### Step 2: claim library construction

Build a library of atomic claims allowed under config (pair claims, count claims, etc.)

```text
def build_claim_library(config: GenerationConfig) -> list[Claim]
```

### Step 3: generate speaker claim bundles consistent with W_star

For each speaker i, we must select a bundle of 1..3 claims such that W_star satisfies the truthfulness rule:
...
* if `W_star[i]` is human: all claims must be true under W_star
* if `W_star[i]` is wolf: at least one claim must be false under W_star

We’ll generate bundles from atomic claims using precomputed truth masks.
...
(Implementation detail: you usually don’t want to enumerate *all* 3-combinations; generate bundles incrementally and sample.)
...
Given speaker i and bundle claims `[c1..ck]`:
...
This enforces `AND(claims) == NOT(W[i])` for that speaker across all candidate assignments.
...
This also guards against bugs in claim evaluation or parsing.
...
Each claim has `to_english(names)`.

A puzzle renderer prints:

* intro story
* list villagers
* for each villager: numbered list of their claims (1..3)
...
* Limit “too strong” claims (e.g., “Exactly 3 werewolves”) via `complexity_cost` and max count-claims
* Avoid duplicate claim_ids across all speakers
* Avoid duplicate bundles
* Ensure diversity: at least X relationship claims total
...
* `claims.py` — claim class hierarchy + english + solver conversion + evaluation
* `claim_factory.py` — canonical serialization + parsing
...
* `build_claim_library(config) -> list[Claim]`
* `ClaimTruthTableCache.build_for_claim_library(...)`
...
* `greedy_assign_claims_until_unique(...) -> Puzzle`
