...
Each villager has a claim on the others:
...
There must be a unique solution, which means a unique set of values for (WA WB WC WD WE WF) that makes the problem coherent for all the claims. I'll call this a Werewolf Problem in the rest of the specs.
...
We will have a function Claim.from_string(str) that returns a claim of the right class for each string, like "W2 => W3" becomes IfAThenB(2, 3)".
...
- Then for each possible claim (for instance, all pairwise relationships for all values of a and b), make the list of truth assignments that the claim is NOT compatible with:

claim_to_imcompatible_assignments = {str(claim): set(
    index
    for truth_assignment, index in truth_assignment_to_index.items()
    if not claim.evaluate_on(truth_assignment)
)}
...
- The rest of the algorithm will consist in finding 3 more claims which are lies (they are incompatible with the ground truth (T, T, T, F, F, F)) but also such that when the claims are attributed to the different characters, the resulting werewolf problem has a unique solution. How this will be done exactly, will depend on the length of the minimal-cover solutions in the previous steps.
