...
That equivalence is correct only if each villager makes exactly one statement (one proposition) and:

humans: their statement is true

werewolves: their statement is false (since “at least one thing they say is wrong” + only one thing said ⇒ that thing must be wrong)

If you ever let villagers make multiple statements, then the werewolf constraint becomes:

werewolf ⇒ NOT(all statements true) (i.e., at least one false)

human ⇒ all statements true

So: your current spec is consistent, just “bakes in” one statement per person.

Modeling note: be explicit about “statements talk about others”

You stated CA = F1(WB, WC, WD, WE, WF) (A talks about others), but later your statement classes allow arbitrary indices. That’s fine, but you should enforce no self-reference at generation time (or decide you allow it).

Also decide whether “CountStatement” counts all 6 or “all except speaker”. Either is fine, but you want it consistent and explicit.

Statement class design: good, with a couple tweaks
...
RelationshipStatement list looks good
...
Count statements are great for variety
...
Your set-cover idea (“pick statements that eliminate all assignments except target”) is basically a hitting set over falsifying assignments. That’s a valid approach for building constraints that uniquely identify a target assignment.

But your puzzles are not “just a conjunction of statements”.
They’re “statements + the meta-rule”:

For each villager i:

If W[i] == False (human), then Statement[i] must evaluate True

If W[i] == True (werewolf), then Statement[i] must evaluate False

So the actual constraint is:

StatementTruth[i] == ¬W[i]

That coupling means you can’t treat statements as independent eliminators unless you incorporate who says what into the coverage.

A cleaner generation viewpoint

Think of a generated puzzle as choosing, for each villager i, a statement Ci from a library such that the system:
...
Decision variables: which statement each villager utters (choose from a finite library)
...
Optional: difficulty / style constraints (no duplicate statement forms, avoid trivial counts, etc.)
...
represent each candidate statement as a Z3 formula

enforce statement(i) == Not(W[i])
...
If your statement library is purely boolean and small, SAT is great.
...
Step A — Precompute statement truth tables (your idea, but include speaker coupling)

For each statement c you can compute the set of assignments where it is True/False.
Store both:
...
Step C — For each villager i, choose a statement consistent with W\*

You need:

if W*[i] = 0 (human) ⇒ choose statement c such that W* ∈ true_set[c]

if W*[i] = 1 (werewolf) ⇒ choose statement c such that W* ∈ false_set[c]
...
After assigning statements to villagers, an assignment W remains viable iff for all i:
...
At each step pick an unassigned villager i and choose among candidate statements the one that removes the most wrong assignments while keeping W\*.

Then finalize with a Z3 uniqueness check (cheap) as a sanity check.

This gives you what you wanted (table-driven) but properly accounts for “the statement truth depends on whether i is wolf”.
...
For each villager i, directly choose a statement that is true/false accordingly

Keep adding statements until uniqueness holds

This way “lies” are not an afterthought; they’re baked in.

Small spec improvements that will save you pain

Decide whether a_index == b_index is allowed for pair statements (usually no).

Add a “no duplicate statement” rule (or a soft penalty), otherwise generation often repeats.

Add a difficulty signal:

fewer count-statements = harder

avoid “There are exactly k werewolves” too often (it can trivialize)

penalize statements that directly mention only one variable (“B is a werewolf”) if you add those later

If you want, I can sketch the exact Z3 uniqueness-check pattern (find model, block it, check sat again) and a greedy generator loop that uses your precomputed incompatible_assignments tables — but the big conceptual fix is: covering assignments must incorporate speaker truthfulness, not just statement truth.
