...
def compute_shill_masks(N: int, M_star: tuple[bool, ...]) -> list[int]:
    """Precompute masks for shill assignments per speaker.
    ...
    Returns:
        ...
        where villager i is a shill according to M_star.

        Each mask is a bitmask where bit j is set if assignment j has
        villager i as a shill (based on M_star).
    ...
    Note:
        ...
        a shill according to M_star.
    """
    num_assignments = 1 << N
    shill_mask_by_speaker = []

    for speaker_index in range(N):
        shill_mask = 0

        # If this speaker is a shill in M_star, all assignments are compatible
        ...
        if M_star[speaker_index]:
            ...
            for assignment_idx in range(num_assignments):
                ...
                if not assignment[speaker_index]:  # Not a werewolf
                    shill_mask |= 1 << assignment_idx
        else:
            # This speaker is not a shill, so no assignments have them as shill
            shill_mask = 0

        shill_mask_by_speaker.append(shill_mask)

    return shill_mask_by_speaker
